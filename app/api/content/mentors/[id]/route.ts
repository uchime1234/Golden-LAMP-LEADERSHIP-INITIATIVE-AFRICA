...
export async function GET(
  ...
  try {
    ...
    const item = await getItem('mentors', id)
    if (!item) {
...
export async function PUT(
  ...
  try {
    ...
    const updated = await updateItem('mentors', id, data)
    if (!updated) {
      return NextResponse.json({ error: 'Mentor not found' }, { status: 404 })
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (permanent) {
      await permanentDelete('mentors', id)
    } else {
      await softDelete('mentors', id)
    }
    invalidateCache('mentors')
...
export async function PATCH(
  ...
  try {
    ...
    if (action === 'restore') {
      const restored = await restoreItem('mentors', id)
      invalidateCache('mentors')
